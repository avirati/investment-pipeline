...
citations, all resolved**. 45.5s wall clock (source 43ms and analyse 45.5s — the
source was warm from the earlier run's HTTP cache), 12 model calls,
79,582 input / 10,732 output tokens on `gpt-4.1-mini`, `cost_usd: null` because
`PRICES` ships empty. GitHub ran **unauthenticated** (`GITHUB_TOKEN` is not set),
so 6 GitHub requests across 12 candidates.
...
calls and spans 54–81. Whether the *ordering* is right is a separate question,
...
score 15/15. Its top band asks for a stated launch date *or* a repository
...
company's rank is driven mostly by how much was *found* about it, which is
uncomfortably close to ranking by GitHub presence. With GitHub unauthenticated
at 6 requests over 12 candidates, that is worth stating plainly.
...
That is a prior artifact *and* a prior role, stated on the record D-1 cites.
...
This is [inconsistency 79](../STATE.md) — *the rubric reads presence, not
meaning* — with a cost attached. It is the same class as
...
overwrote the manifest (worklog 0039, what went wrong #3). Same seed, same
...
*"reproducibility comes from the response cache, not from a sampler setting."*
...
TODO(author) — the `ardent` finding and the run-to-run variance point at the
same seam from two directions: everything downstream of the extractor is
deterministic and auditable, and the extractor is neither. Worth a paragraph on
whether that seam was in the right place, and on what the honest thing to tell a
partner about these twelve calls actually is.
